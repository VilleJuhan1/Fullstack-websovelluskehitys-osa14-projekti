...
resource "oci_budget_budget" "project_budget" {
  ...
  targets        = [oci_identity_compartment.project[each.key].id]
}

resource "oci_budget_alert_rule" "project_budget_alert" {
  for_each       = var.projects
  budget_id      = oci_budget_budget.project_budget[each.key].id
  type           = "FORECAST"
  threshold      = 100
...
resource "oci_events_rule" "security_events" {
  ...
  display_name   = "${each.value.name}-SecurityEvents"
  is_enabled     = true
...
resource "oci_logging_log_group" "network_log_group" {
  for_each       = var.projects
  compartment_id = oci_identity_compartment.network[each.key].id
